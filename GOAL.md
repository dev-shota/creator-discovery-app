...
- **デプロイ(Cloudflare)地雷: `package-lock.json` を更新するときは必ず npm 10 で**（`npx npm@10 install`）。npm 11 は dedup/optional 依存エントリを削り、Cloudflare の `npm ci`(Linux) を壊す＝ビルド失敗・旧ビルドのまま反映されない（2026-06-14 再発。npm10 再同期で +156 行復活して解消）。lock を触ったら必ず実デプロイで反映を確認する（live HTML を grep する等）。
