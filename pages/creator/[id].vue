<template>
  <div class="seo-page">
    ...
    <main class="seo-main">
      <h1>{{ staffName }}の作品</h1>
      <p class="seo-desc">{{ staffName }}の漫画・アニメ作品一覧。好きな作り手から「次に見る一本」をたどって見つけよう。</p>
      <a :href="`/?view=creator&id=${id}`" class="seo-cta">作品を見る →</a>
    </main>
  </div>
</template>
...
const staffName = computed(() => {
  ...
  return n?.native || n?.full || '作り手'
})

const SITE_URL = config.public.siteUrl as string
const title = computed(() => `${staffName.value}の作品 — Creator Discovery`)
const desc = computed(() => `${staffName.value}の漫画・アニメ作品一覧。高評価順・おすすめ付き。Creator Discoveryで見つけよう。`)

useHead({
  title,
  link: [{ rel: 'canonical', href: `${SITE_URL}/creator/${id}` }],
  script: [{
    ...
    innerHTML: JSON.stringify({
      ...
      mainEntity: { '@type': 'Person', name: staffName.value },
    }),
  }],
})
useSeoMeta({
  ...
  ogTitle: title,
  ogDescription: desc,
  ogUrl: `${SITE_URL}/creator/${id}`,
  ogImage: `${SITE_URL}/og-image.png`,
  twitterCard: 'summary_large_image',
})

if (import.meta.client) {
  navigateTo({ path: '/', query: { view: 'creator', id: String(id) } }, { replace: true })
}
</script>
