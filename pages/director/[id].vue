<template>
  <div class="seo-page">
    ...
    <main class="seo-main">
      <h1>{{ staffName }}の監督作品</h1>
      <p class="seo-desc">{{ staffName }}が監督したアニメ作品一覧。高評価順・おすすめ付き。Creator Discoveryで見つけよう。</p>
      <a :href="`/?view=director&id=${id}`" class="seo-cta">作品を見る →</a>
    </main>
  </div>
</template>
...
useSeoMeta({
  ...
  ogTitle: title,
  ogDescription: desc,
  ogUrl: `${SITE_URL}/director/${id}`,
  ogImage: `${SITE_URL}/og-image.png`,
  twitterCard: 'summary_large_image',
})

if (import.meta.client) {
  navigateTo({ path: '/', query: { view: 'director', id: String(id) } }, { replace: true })
}
</script>
