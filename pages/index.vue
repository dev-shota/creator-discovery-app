...
type RecentKind = 'creator' | 'title' | 'director' | 'voice' | 'writing' | 'chardesign' | 'music' | 'theme-singer' | 'theme-lyrics' | 'theme-compose' | 'studio'
...
type SearchMode = 'creator' | 'title' | 'director' | 'voice' | 'writing' | 'chardesign' | 'music' | 'theme-singer' | 'theme-lyrics' | 'theme-compose' | 'studio'
...
const recentByKind = ref<Record<RecentKind, RecentItem[]>>({ creator: [], title: [], director: [], voice: [], writing: [], chardesign: [], music: [], 'theme-singer': [], 'theme-lyrics': [], 'theme-compose': [], studio: [] })
...
type RoleKey = 'writing' | 'chardesign' | 'creative' | 'music' | 'theme-singer' | 'theme-lyrics' | 'theme-compose'
...
function loadAllRecent() {
  recentByKind.value = {
    ...
    music: loadRecentKind('music'),
    'theme-singer': loadRecentKind('theme-singer'),
    'theme-lyrics': loadRecentKind('theme-lyrics'),
    'theme-compose': loadRecentKind('theme-compose'),
...
// AniList Staff.role の文字列を日本語の職能ラベルに変換。
// 2026-06-15 拡張（user req）: アートディレクター等の他職種も collaborator カードに正しい
// 日本語で出るように。判定順序は specific → general（"Chief Animation Director" を
// "Animation Director" より先、"Photography Director" を "Photography" より先）。
// クリック先ルーティング（goToCollaborator）は主要credit以外を creator モードに落として
// AniList Staff の代表作にフォールバック（職種専用モードは未配線のため）。
...
// collaborator カードに見せる代表ロール。同一人物が複数 role を兼ねるとき、主要credit を
// 優先（監督・シリーズ構成等）して落ちる場合のみ他職種から拾う。
function pickBestRole(roles: string[]): string {
  const tests: ((r: string) => boolean)[] = [
    ...
    r => /Producer/.test(r),
    r => /Original/.test(r),
    // 他職種（主要credit以下）。Chief→Animation Director を Animation Director より先。
