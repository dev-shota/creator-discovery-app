<template>
  <div class="seo-page">
    ...
    <main class="seo-main">
      <h1>{{ staffName }}の音楽担当作品</h1>
      <p class="seo-desc">{{ staffName }}が音楽を担当したアニメ作品一覧。高評価順・おすすめ付き。Creator Discoveryで見つけよう。</p>
      <a :href="`/?view=music&id=${id}`" class="seo-cta">担当作品を見る →</a>
    </main>
  </div>
</template>
...
useSeoMeta({
  ...
  ogTitle: title,
  ogDescription: desc,
  ogUrl: `${SITE_URL}/music/${id}`,
  ogImage: `${SITE_URL}/og-image.png`,
  twitterCard: 'summary_large_image',
})

if (import.meta.client) {
  navigateTo({ path: '/', query: { view: 'music', id: String(id) } }, { replace: true })
}
</script>
