<template>
  <div class="seo-page">
    ...
    <main class="seo-main">
      <h1>{{ studioName }}の制作作品</h1>
      <p class="seo-desc">{{ studioName }}が制作したアニメ作品一覧。高評価順・おすすめ付き。Creator Discoveryで見つけよう。</p>
      <a :href="`/?view=studio&id=${id}`" class="seo-cta">作品を見る →</a>
    </main>
  </div>
</template>
...
const studioName = computed(() => nameCache.value?.studioNames?.[id] || '制作会社')
...
useSeoMeta({
  ...
  ogTitle: title,
  ogDescription: desc,
  ogUrl: `${SITE_URL}/studio/${id}`,
  ogImage: `${SITE_URL}/og-image.png`,
  twitterCard: 'summary_large_image',
})

if (import.meta.client) {
  navigateTo({ path: '/', query: { view: 'studio', id: String(id) } }, { replace: true })
}
</script>
