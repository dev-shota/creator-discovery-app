<template>
  <div class="seo-page">
    ...
    <main class="seo-main">
      <h1>{{ staffName }}の作曲・編曲作品</h1>
      <p class="seo-desc">{{ staffName }}が作曲・編曲を担当したアニメ作品一覧。高評価順・おすすめ付き。Creator Discoveryで見つけよう。</p>
      <a :href="`/?view=theme-compose&id=${id}`" class="seo-cta">担当作品を見る →</a>
    </main>
  </div>
</template>
...
const staffName = computed(() => {
  ...
  return n?.native || n?.full || '作曲・編曲家'
})

const SITE_URL = config.public.siteUrl as string
const title = computed(() => `${staffName.value}の作曲・編曲作品 — Creator Discovery`)
const desc = computed(() => `${staffName.value}が作曲・編曲を担当したアニメ一覧。高評価順・おすすめ付き。`)

useHead({
  title,
  link: [{ rel: 'canonical', href: `${SITE_URL}/theme-compose/${id}` }],
  script: [{
    ...
    innerHTML: JSON.stringify({
      ...
      url: `${SITE_URL}/theme-compose/${id}`,
      mainEntity: { '@type': 'Person', name: staffName.value, jobTitle: 'Composer/Arranger' },
    }),
  }],
})
useSeoMeta({
  ...
  ogTitle: title,
  ogDescription: desc,
  ogUrl: `${SITE_URL}/theme-compose/${id}`,
  ogImage: `${SITE_URL}/og-image.png`,
  twitterCard: 'summary_large_image',
})

if (import.meta.client) {
  navigateTo({ path: '/', query: { view: 'theme-compose', id: String(id) } }, { replace: true })
}
</script>
