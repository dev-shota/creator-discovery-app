<template>
  <div class="seo-page">
    ...
    <main class="seo-main">
      <h1>{{ staffName }}のOP/ED出演作品</h1>
      <p class="seo-desc">{{ staffName }}がOP/EDを歌ったアニメ作品一覧。高評価順・おすすめ付き。Creator Discoveryで見つけよう。</p>
      <a :href="`/?view=theme-singer&id=${id}`" class="seo-cta">出演作品を見る →</a>
    </main>
  </div>
</template>
...
const staffName = computed(() => {
  ...
  return n?.native || n?.full || 'OP/ED歌手'
})
...
useSeoMeta({
  ...
  ogTitle: title,
  ogDescription: desc,
  ogUrl: `${SITE_URL}/theme-singer/${id}`,
  ogImage: `${SITE_URL}/og-image.png`,
  twitterCard: 'summary_large_image',
})

if (import.meta.client) {
  navigateTo({ path: '/', query: { view: 'theme-singer', id: String(id) } }, { replace: true })
}
</script>
